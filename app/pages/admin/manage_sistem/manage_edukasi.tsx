...
export default function ManageEdukasi() {
    ...
    const [saving, setSaving] = useState(false); // ✅ pisah dari loading
    ...
    const openAddEdukasi = () => {
        ...
        setSolusi(''); setImage(null); setExistingImage(null);
    ...
    const handleSaveEdukasi = async () => {
        // ✅ Validasi pakai showToast
        if (!name.trim() || !description.trim()) {
            showToast('Nama dan deskripsi wajib diisi!', 'error'); return;
        }
        if (!editingId && !image) {
            showToast('Gambar wajib dipilih!', 'error'); return;
        }
        ...
        try {
            ...
            setSaving(false); // ✅ pakai saving
        }
    };

    // ✅ handleDeleteEdukasi pakai showConfirm
    const handleDeleteEdukasi = (id: number, nama: string) => {
        showConfirm({
            visible: true,
            title: 'Hapus Data Edukasi',
            message: `Yakin hapus "${nama}"? Semua video terkait juga akan dihapus.`,
            confirmText: 'Hapus',
            confirmColor: '#DC2626',
            icon: 'trash-outline',
            iconColor: '#DC2626',
    ...
    const handleSaveVideo = async () => {
        // ✅ Validasi pakai showToast
        if (!judulVideo.trim()) {
            showToast('Judul video wajib diisi!', 'error'); return;
        }
        if (!editingVideoId && tipeVideo === 'link' && !videoLink.trim()) {
            showToast('Link video wajib diisi!', 'error'); return;
        }
        if (!editingVideoId && tipeVideo === 'file' && !videoFile) {
            showToast('File video wajib dipilih!', 'error'); return;
        }
        ...
        try {
            ...
            formData.append('video', videoLink);
                ...
                if (videoFile.uri.startsWith('blob:') || videoFile.uri.startsWith('data:')) {
                    // ✅ Web: buat File object dengan type eksplisit
                    const res = await fetch(videoFile.uri);
                    const blob = await res.blob();
                    const ext = videoFile.name.split('.').pop() || 'mp4';
                    const mimeType = ext === 'mp4' ? 'video/mp4' : ext === 'mov' ? 'video/quicktime' : 'video/mp4';
                    const file = new File([blob], videoFile.name, { type: mimeType });
                    formData.append('video', file, videoFile.name);
                } else {
                    // Native
            ...
            if (response.ok) {
                setModalVisible(false);
                showToast(editingVideoId ? 'Video berhasil diperbarui!' : 'Video berhasil ditambahkan!', 'success');
                fetchData();
            } else {
                showToast(result?.message || `Gagal menyimpan (${response.status})`, 'error');
            }
    ...
    // ✅ handleDeleteVideo pakai showConfirm
    const handleDeleteVideo = (v: VideoEdukasi) => {
        showConfirm({
            visible: true,
            title: 'Hapus Video',
            message: `Yakin hapus video "${v.judul_video}"?`,
            confirmText: 'Hapus',
            confirmColor: '#DC2626',
            icon: 'trash-outline',
            iconColor: '#DC2626',
    ...
    return (
        // ✅ Root View biasa, bukan SafeAreaView
        <View style={{ flex: 1 }}>
                        ...
                        <ScrollView keyboardShouldPersistTaps="handled">
                            ...
                            <View style={styles.modalBody}>
                                ...
                                <View style={styles.modalFooter}>
                                    ...
                                    <TouchableOpacity style={styles.btnSave} disabled={saving}
...
const styles = StyleSheet.create({
    ...
    infoChip: { flexDirection: 'row', alignItems: 'center', gap: 3, marginTop: 3, paddingHorizontal: 5, paddingVertical: 2, borderRadius: 5, alignSelf: 'flex-start', maxWidth: '100%' },
    infoChipText: { fontSize: 10, fontWeight: '600', flexShrink: 1 },
