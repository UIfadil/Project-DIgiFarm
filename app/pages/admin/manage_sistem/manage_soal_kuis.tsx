...
export default function ManageSoalKuis() {
    ...
    const [saving, setSaving] = useState(false); // ✅ pisahkan loading save
    ...
    const showConfirm = (config: ConfirmDialogState) => setConfirmDialog(config);
    ...
    const handleRemoveImage = () => {
        showConfirm({
            visible: true,
            title: 'Hapus Gambar',
            message: 'Yakin ingin menghapus gambar soal ini?',
            confirmText: 'Hapus',
            confirmColor: '#DC2626',
            icon: 'image-outline',
            iconColor: '#DC2626',
    ...
    const handleSave = async () => {
        if (!pertanyaan.trim()) { showToast('Pertanyaan wajib diisi!', 'error'); return; }
        if (!opsiA.trim() || !opsiB.trim() || !opsiC.trim() || !opsiD.trim()) {
            showToast('Semua opsi jawaban wajib diisi!', 'error'); return;
        }
        ...
        try {
            ...
            if (image) {
                if (image.startsWith('blob:') || image.startsWith('data:')) {
                    ...
                    formData.append('gambar', file, filename);
                } else {
            ...
            if (response.ok) {
                ...
                showToast(editingId ? 'Soal berhasil diperbarui!' : 'Soal berhasil ditambahkan!', 'success');
                fetchData();
            } else {
                showToast(result?.message || `Gagal menyimpan (${response.status})`, 'error');
            }
        } catch (error: any) {
    ...
    const handleDelete = (id: number, pertanyaanText: string) => {
        showConfirm({
            visible: true,
            title: 'Hapus Soal',
            message: `Yakin hapus soal ini?\n"${pertanyaanText.substring(0, 60)}${pertanyaanText.length > 60 ? '...' : ''}"`,
            confirmText: 'Hapus',
            confirmColor: '#DC2626',
            icon: 'trash-outline',
            iconColor: '#DC2626',
    ...
    const renderItem = ({ item, index }: { item: SoalKuis; index: number }) => {
        ...
        return (
            <View style={[styles.card, index % 2 === 0 && styles.cardEven]}>
                ...
                {item.gambar ? (
                    <Image source={{ uri: getImageUri(item.gambar) }} style={styles.soalImage} resizeMode="cover" />
                ) : null}
                        ...
                        return (
                            <View key={key} style={[styles.opsiRow, isBenar && styles.opsiRowBenar]}>
                                <View style={[styles.opsiHuruf, isBenar && styles.opsiHurufBenar]}>
                                    <Text style={[styles.opsiHurufText, isBenar && styles.opsiHurufTextBenar]}>{key.toUpperCase()}</Text>
                                </View>
                                <Text style={[styles.opsiText, isBenar && styles.opsiTextBenar]} numberOfLines={2}>{opsiText}</Text>
    ...
    return (
        <View style={{ flex: 1 }}>
                    ...
                    <SafeAreaView style={{ flex: 1, backgroundColor: '#F8FAFC' }}>
                        <ScrollView keyboardShouldPersistTaps="handled">
                            <View style={styles.modalHeader}>
                                <View style={{ flex: 1 }}>
                                    <Text style={styles.modalTitle}>{editingId ? '✏️ Edit Soal' : '➕ Tambah Soal Baru'}</Text>
                                ...
                                <View style={styles.sectionBox}>
                                        ...
                                        {(['hama', 'penyakit'] as Kategori[]).map(cat => {
                                            ...
                                            return (
                                                <TouchableOpacity key={cat} onPress={() => setKategori(cat)}
                                                    ...
                                                    <Text style={[styles.catText, kategori === cat && { color: cfg.text, fontWeight: '700' }]}>{cfg.label}</Text>
                                    ...
                                    <View style={[styles.categoryRow, { marginTop: 8 }]}>
                                        <TouchableOpacity onPress={() => setKategori('pertanian_dasar')}
                                            ...
                                            <Text style={[styles.catText, kategori === 'pertanian_dasar' && { color: '#854D0E', fontWeight: '700' }]}>Pertanian Dasar</Text>
                                    ...
                                    {([
                                        ...
                                        return (
                                            <View key={key} style={styles.opsiInputRow}>
                                                <TouchableOpacity onPress={() => setJawabanBenar(key)}
                                                    style={[styles.opsiHurufInput, isBenar && styles.opsiHurufInputActive]}>
                                                    <Text style={[styles.opsiHurufInputText, isBenar && styles.opsiHurufInputTextActive]}>{key.toUpperCase()}</Text>
                                                </TouchableOpacity>
                                                <TextInput
                                                ...
                                                />
                                                {isBenar && <Ionicons name="checkmark-circle" size={20} color="#16A34A" style={{ marginLeft: 6 }} />}
                                            </View>
                                        );
                                    })}
                                    <View style={styles.jawabanHint}>
                                        <Ionicons name="information-circle-outline" size={14} color="#6366F1" />
                                        <Text style={styles.jawabanHintText}>Ketuk huruf (A/B/C/D) untuk menandai jawaban yang benar</Text>
                                    </View>
                                </View>
                                    ...
                                    >
                                        {saving
                                            ...
                                            : <>
                                                ...
                                                <Text style={styles.btnSaveText}>{editingId ? 'Simpan Perubahan' : 'Simpan Soal'}</Text>
                                            </>
                                        }
                                    </TouchableOpacity>
                                </View>
            ...
            {/* ✅ Di LUAR SafeAreaView agar tidak terhalang Modal */}
            <ConfirmDialog
